...
func (s *storage) CreateAPIKey(ctx context.Context, key *APIKey) error {
	...
	query := `
        INSERT INTO api_keys (id, user_id, name, key_hash, key_value, prefix, 
                              expires_at, created_at, created_by)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    `

	_, err := s.db.ExecContext(ctx, query,
		key.ID, key.UserID, key.Name, key.KeyHash, key.KeyValue, key.Prefix,
...
func (s *storage) GetAPIKeyByHash(ctx context.Context, keyHash string) (*APIKey, error) {
	query := `
        SELECT id, user_id, name, key_hash, key_value, prefix, last_used_at, 
